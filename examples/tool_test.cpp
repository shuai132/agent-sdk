...
int main() {
    ...
    agent_cfg.system_prompt = "You are a helpful assistant. When asked to list files, use the glob tool with pattern '*' to list files in the current directory. Be concise.";
    ...
    session->set_permission_handler(
        [](const std::string& perm, const std::string& desc) {
    ...
    // Wait for completion with timeout
    {
        ...
        if (!cv.wait_for(lock, std::chrono::seconds(120), [&]{ return completed; })) {
