...
BashTool::BashTool() : SimpleTool("bash", 
    "Executes a given bash command in a persistent shell session with optional timeout.") {}

std::vector<ParameterSchema> BashTool::parameters() const {
    return {
        {"command", "string", "The command to execute", true, std::nullopt, std::nullopt},
...
ReadTool::ReadTool() : SimpleTool("read",
...
std::vector<ParameterSchema> ReadTool::parameters() const {
    return {
        {"filePath", "string", "The absolute path to the file to read", true, std::nullopt, std::nullopt},
...
WriteTool::WriteTool() : SimpleTool("write",
...
std::vector<ParameterSchema> WriteTool::parameters() const {
    return {
        {"filePath", "string", "The absolute path to the file to write", true, std::nullopt, std::nullopt},
...
std::future<ToolResult> WriteTool::execute(const json& args, const ToolContext& ctx) {
    return std::async(std::launch::async, [args, ctx]() -> ToolResult {
        ...
        return ToolResult::with_title(
            "Successfully wrote " + std::to_string(content.size()) + " bytes to " + path.string(),
...
EditTool::EditTool() : SimpleTool("edit",
    "Performs exact string replacements in files using search and replace.") {}

std::vector<ParameterSchema> EditTool::parameters() const {
    return {
        {"filePath", "string", "The absolute path to the file to modify", true, std::nullopt, std::nullopt},
    ...
    return std::async(std::launch::async, [args, ctx]() -> ToolResult {
        ...
        if (count > 1 && !replace_all) {
            return ToolResult::error(
                "oldString found " + std::to_string(count) + " times. " +
        ...
        return ToolResult::with_title(
            "Replaced " + std::to_string(replaced) + " occurrence(s) in " + path.string(),
...
GlobTool::GlobTool() : SimpleTool("glob",
    "Fast file pattern matching tool. Supports glob patterns like \"**/*.js\".") {}

std::vector<ParameterSchema> GlobTool::parameters() const {
    return {
        {"pattern", "string", "The glob pattern to match files against", true, std::nullopt, std::nullopt},
...
std::future<ToolResult> GlobTool::execute(const json& args, const ToolContext& ctx) {
    return std::async(std::launch::async, [args, ctx]() -> ToolResult {
            ...
            for (const auto& entry : fs::recursive_directory_iterator(base_path)) {
                ...
                if (pattern.find("**") != std::string::npos) {
                    ...
                    if (filename.size() >= ext_pattern.size() &&
                        filename.compare(filename.size() - ext_pattern.size(), ext_pattern.size(), ext_pattern) == 0) {
                        match = true;
                    }
        ...
        return ToolResult::with_title(
            output.str(),
            "Found " + std::to_string(matches.size()) + " files"
        );
    });
}
...
GrepTool::GrepTool() : SimpleTool("grep",
...
std::vector<ParameterSchema> GrepTool::parameters() const {
    return {
        {"pattern", "string", "The regex pattern to search for", true, std::nullopt, std::nullopt},
        {"path", "string", "The directory to search in", false, std::nullopt, std::nullopt},
...
QuestionTool::QuestionTool() : SimpleTool("question",
...
std::vector<ParameterSchema> QuestionTool::parameters() const {
    return {
        {"questions", "array", "Array of questions to ask", true, std::nullopt, std::nullopt}
    };
}
...
TaskTool::TaskTool() : SimpleTool("task",
    "Launch a new agent to handle complex, multistep tasks autonomously.") {}

std::vector<ParameterSchema> TaskTool::parameters() const {
    return {
        {"prompt", "string", "The task for the agent to perform", true, std::nullopt, std::nullopt},
        {"description", "string", "A short description of the task", true, std::nullopt, std::nullopt},
        {"subagent_type", "string", "The type of agent to use", true, std::nullopt, 
            std::vector<std::string>{"general", "explore"}},
