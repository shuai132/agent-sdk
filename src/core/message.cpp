...
void Message::add_tool_result(const std::string& call_id, const std::string& name,
    ...
    if (j.contains("parts")) {
        for (const auto& part_json : j["parts"]) {
            ...
            if (type == "text") {
                ...
                msg.parts_.push_back(ToolCallPart{
                    part_json["id"],
                    part_json["name"],
                    part_json["arguments"],
                    part_json.value("started", false),
                    part_json.value("completed", false)
                });
            } else if (type == "tool_result") {
                msg.parts_.push_back(ToolResultPart{
                    part_json["tool_call_id"],
                    part_json["tool_name"],
                    part_json["output"],
                    part_json.value("is_error", false),
                    std::nullopt,
                    json::object(),
                    part_json.value("compacted", false),
                    std::nullopt
                });
            }
        }
...
void InMemoryMessageStore::remove(const MessageId& id) {
    ...
    if (it != messages_.end()) {
        ...
        session_msgs.erase(
            std::remove(session_msgs.begin(), session_msgs.end(), id),
            session_msgs.end()
        );
        messages_.erase(it);
    }
