...
private:
                        ...
                        socket->async_handshake(
                            ...
                            [this, socket, request_str, buffer, status_code, shared_on_data, shared_on_complete](
                                ...
                                asio::async_write(
                                    *socket,
                                    asio::buffer(*request_str),
                                    [this, socket, buffer, status_code, shared_on_data, shared_on_complete](
                                        const asio::error_code& ec,
                                        size_t
                                    ) {
                                        if (ec) {
    ...
    template<typename Socket>
    void read_stream_headers(
        std::shared_ptr<Socket> socket,
        std::shared_ptr<asio::streambuf> buffer,
        std::shared_ptr<int> status_code,
        std::shared_ptr<StreamDataCallback> on_data,
        std::shared_ptr<std::function<void(int, const std::string&)>> on_complete
    ) {
        asio::async_read_until(
            *socket, *buffer, "\r\n\r\n",
            [this, socket, buffer, status_code, on_data, on_complete](
                const asio::error_code& ec,
                size_t bytes_transferred
            ) {
                if (ec && ec != asio::error::eof) {
...
void HttpClient::request_stream(
    const std::string& url,
    const HttpOptions& options,
    StreamDataCallback on_data,
