...
public:
    ...
    void connect(
        ...
        std::string port = match[3].str().empty() ? 
            (is_https ? "443" : "80") : match[3].str();
...
private:
    void connect_https(const std::string& host, const std::string& port) {
        ...
        resolver_.async_resolve(host, port,
            [this](const asio::error_code& ec, auto results) {
                if (ec || stopped_) {
                    if (on_error_) on_error_("DNS resolution failed: " + ec.message());
                    return;
                }
                
                asio::async_connect(ssl_socket_->lowest_layer(), results,
                    [this](const asio::error_code& ec, auto) {
                        if (ec || stopped_) {
    ...
    void connect_http(const std::string& host, const std::string& port) {
        ...
        resolver_.async_resolve(host, port,
            [this](const asio::error_code& ec, auto results) {
                ...
                asio::async_connect(*tcp_socket_, results,
                    [this](const asio::error_code& ec, auto) {
                        if (ec || stopped_) {
                            if (on_error_) on_error_("Connection failed: " + ec.message());
                            return;
    ...
    void send_request_tcp() {
        asio::async_write(*tcp_socket_, asio::buffer(request_),
            [this](const asio::error_code& ec, size_t) {
                if (ec || stopped_) {
                    if (on_error_) on_error_("Write failed: " + ec.message());
                    return;
...
SseClient::SseClient(asio::io_context& io_ctx)
